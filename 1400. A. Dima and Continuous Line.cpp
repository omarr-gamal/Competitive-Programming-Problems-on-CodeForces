// link to problem: https://codeforces.com/problemset/problem/358/A
...
void solve() {
    ...
    for (int i = 0; i < n - 1; ++i) {
        double x, r;
        x = ((double)v[i] + (double)v[i + 1]) / 2;
        r = abs(x - v[i]);

        for (auto c: circles) {
            double pos_difference = abs((double)c.first - x);
            if ((pos_difference < abs(r + c.second)) && (pos_difference > abs(r - c.second))) {
                goto label;
            }
        }
